 ...
 *
 * Вам предстоит проследить механизм обмена генов между особями, а так же осознать могущество всеобъемлющей случайности
 * в жизни каждого живого существа. Вы не будете иметь представления об именах особей подопытного вида, о внешности и т.д.,
 * потому как с точки зрения эволюции каждая особь каждого вида - это просто безымянный набор генов.Ни больше, ни меньше.
 ...
 * 4, 5, 6 ,7 - четыре случайных гена (например: цвет глаз, скорость роста, степень оволосения, эффективность механизма регенерации и т.д.).
 * Здесь следует упомянуть, что генов - многие тысячи, но для демонстрации механизма обмена хватит и четырёх.
 ...
 * Так же очень велика вероятность, что вредная генная мутация убъёт особь ещё до полового созревания.
 ...
 * 2.Количество мужских и женских особей у этого вида всегда равно 
 *(небольшое расхождение с жизнью, в которой мужская популяция всегда немного преобладает).
 * 3.Количество детей у каждой пары : 3 - 6 (не очень плодовитый вид).
 *
 * Объяснение алгоритма работы кода (необязательная часть):
 * 1.Создаётся стартовое поколение(массив) из 10-ти особей(массивов).Они попадают под полноценный естественный отбор.
 * 2.С вероятностью 45% каждая особь до оставления потомства не доживает ввиду различных причин. Массив чистится от мёртвых особей.
 ...
 * 4.Случайным образом определяется кол - во детей у каждой пары(3-6), и каждому ребёнку этой пары даются гены родителей.
 * Затем ребёнок вносится в новый массив следующего поколения. Если он получил летальную мутацию, то "вычищается" из списка.
 * 5.Первоначальный массив замещается новым, что позволяет новому пройти тот же круг.
 *
